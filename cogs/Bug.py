...
df = pd.read_csv(filedir + '/bug_data.csv')
...
class Bug(commands.Cog):
    ...
    async def on_ready(self):
        print(f'Bug Cog is online')

    @commands.command()
    async def bug(self, ctx, *, bug: str):
        if bug.lower() in v_name2:
            idx = v_name2.index(bug.lower())
        elif bug.lower() in v_search:
            idx = v_search.index(bug.lower())
        else:
            return
    ...
    @commands.command()
    async def bug_month(self, ctx, month):
        ...
        q1 = df[(df.Rarity == 'common') & (df[month]==True)] # queries for common bugs of the requested month
        q1 = list(q1.Name) # turns the dataframe column of bug names into a list
